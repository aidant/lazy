...
export interface Provider<T, TContext extends Context = Record<string, never>> {
  token: Token<T>
...
export namespace Provider {
  export type Type<TProvider extends Provider<unknown>> = TProvider extends { value?: infer T }
    ? T
    : never
}

export type Providers = { [K in string]: Provider<unknown> }
...
export function useFactoryWithArgs<T, TArgs extends readonly [Token<unknown>, ...Token<unknown>[]]>(
  ...
  return {
    ...
    factory: (injectionContext) =>
      factory(
        ...(args.map((token) => injectionContext.injector.get(token)) as {
