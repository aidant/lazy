...
Sql.prototype[inspect] = function (_depth: number, options: object, inspect: Function): unknown {
  ...
  const query = new QueryInput(mutable.text, mutable.parametersByName, mutable.parametersByPosition)

  type Deletable<T> = { -readonly [P in keyof T]?: T[P] }

  if (!query.text) delete (query as Deletable<QueryInput>).text
  if (!query.parametersByName) delete (query as Deletable<QueryInput>).parametersByName
  if (!query.parametersByPosition) delete (query as Deletable<QueryInput>).parametersByPosition
